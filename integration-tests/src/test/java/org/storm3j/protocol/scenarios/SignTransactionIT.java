...
import org.storm3j.protocol.core.methods.response.FstSign;
...
/** Sign transaction using Fst node. */
public class SignTransactionIT extends Scenario {
    ...
    public void testSignTransaction() throws Exception {
        ...
        FstSign fstSign =
                storm3j.fstSign(ALICE.getAddress(), Numeric.toHexString(hashed)).sendAsync().get();

        String signature = fstSign.getSignature();
        assertNotNull(signature);
        assertFalse(signature.isEmpty());
    }

    private static RawTransaction createTransaction() {
        BigInteger value = Convert.toWei("1", Convert.Unit.FST).toBigInteger();
