 ...
 */
package org.storm3j.protocol.besu.response.privacy;
...
import org.storm3j.utils.Restriction;
    ...
    public static class ResponseDeserialiser extends StdDeserializer<PrivateTransaction> {
        ...
        public PrivateTransaction deserialize(JsonParser p, DeserializationContext ctxt)
            ...
            if (node.get("privateFor").isArray()) {
                return p.getCodec().treeToValue(node, org.storm3j.protocol.besu.response.privacy.PrivateTransactionLegacy.class);
            } else if (node.get("privateFor").isValueNode()) {
                return p.getCodec().treeToValue(node, org.storm3j.protocol.besu.response.privacy.PrivateTransactionWithPrivacyGroup.class);
            }
