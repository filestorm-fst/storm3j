...
import static org.storm3j.protocol.http.HttpService.JSON_MEDIA_TYPE;

/** Protocol Response tests. */
public abstract class ResponseTester {

    private HttpService storm3jService;
    ...
    protected void configureWeb3Service(boolean includeRawResponses) {
        storm3jService = new HttpService(okHttpClient, includeRawResponses);
    }

    protected <T extends Response> T deserialiseResponse(Class<T> type) {
        T response = null;
        try {
            response = storm3jService.send(new Request(), type);
        } catch (IOException e) {
            fail(e.getMessage());
        }
        return response;
